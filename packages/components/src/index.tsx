...
export type { CardGridProps, LinkCardProps } from "./LinkCard.js";
// ── LINK CARD & CARD GRID (Phase 3.1) ──────────────────
export { CardGrid, LinkCard } from "./LinkCard.js";
...
export function Callout({ type = "info", title, children }: CalloutProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 14, lineHeight: 1.65, color: "var(--tx2)" }}>
        {children}
      </div>
    </div>
  );
...
export function Tabs({ items, children }: TabsProps) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div
        style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--bd)" }}
      >
        {items.map((item, i) => (
          <button
            key={i}
      ...
      <div style={{ padding: "16px 0" }}>{children[active]}</div>
    </div>
  );
}
...
export function Card({ title, icon, href, children }: CardProps) {
  const resolvedIcon =
    typeof icon === "string" && CARD_ICONS[icon] ? CARD_ICONS[icon] : icon;
  const content = (
    <div
      onMouseOut={(e) => {
    ...
    >
      {resolvedIcon && (
        <span style={{ display: "block", marginBottom: 8, color: "var(--ac)" }}>
          {resolvedIcon}
        </span>
      )}
      ...
      {children && (
        <div style={{ fontSize: 13, color: "var(--txM)", lineHeight: 1.5 }}>
          {children}
        </div>
      )}
    </div>
        ...
        >
          <thead>
            <tr style={{ borderBottom: "2px solid var(--bd)" }}>
              {["Property", "Type", "Required", "Default", "Description"].map(
                (h) => (
                  <th
                    key={h}
                    style={{
          ...
          <tbody>
            {fields.map((f) => (
              <tr key={f.name} style={{ borderBottom: "1px solid var(--bd)" }}>
                ...
                <td style={{ padding: "8px 10px", color: "var(--tx2)" }}>
                  {f.description || ""}
                </td>
              </tr>
            ))}
