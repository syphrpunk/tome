...
describe("convertDocusaurusContent", () => {
  ...
  it("strips @docusaurus/ imports", () => {
    const input = "import Link from '@docusaurus/Link';\n\n# Hello\n";
    const { converted } = convertDocusaurusContent(input);
...
describe("migrateFromDocusaurus", () => {
  ...
  it("handles subdirectories in docs/", async () => {
    ...
    expect(result.convertedFiles.some((f) => f.includes("guides"))).toBe(true);
    expect(existsSync(join(out, "pages", "guides", "getting-started.md"))).toBe(
      true
    );
  });
});
