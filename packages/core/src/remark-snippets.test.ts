...
const TMP_DIR = resolve(import.meta.dirname, "../.test-tmp-snippets");
...
async function processWithSnippets(
  markdown: string,
...
describe("resolveSnippet", () => {
  ...
  it("throws for missing snippet file", () => {
    expect(() => resolveSnippet(SNIPPETS_DIR, "nonexistent.md")).toThrow(
      "Snippet not found"
    );
  });
...
describe("remarkSnippets", () => {
  ...
  it("substitutes variables in snippet content", async () => {
    writeSnippet(
      "versioned.md",
      "You are using version {{version}} of the API."
    );
    const result = await processWithSnippets(
      '::snippet{file="versioned.md" version="v2"}'
    );
  ...
  it("substitutes multiple variables", async () => {
    ...
    const result = await processWithSnippets(
      '::snippet{file="multi-var.md" name="Tome" version="1.0"}'
    );
  ...
  it("preserves unmatched variables", async () => {
    ...
    const result = await processWithSnippets(
      '::snippet{file="partial-var.md" name="Alice"}'
    );
  ...
  it("shows error message for missing snippet", async () => {
    const result = await processWithSnippets(
      '::snippet{file="does-not-exist.md"}'
    );
  ...
  it("respects max depth to prevent infinite recursion", async () => {
    ...
    const result = await processWithSnippets('::snippet{file="recursive.md"}', {
      maxDepth: 2,
    });
