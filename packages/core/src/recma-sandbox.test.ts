...
describe("recma-sandbox", () => {
  ...
  describe("dangerous patterns (should fail)", () => {
    it("blocks fetch() calls", () => {
      const tree = programWith(
        exprStmt(callExpr(id("fetch"), [literal("https://evil.com")]))
      );
      const result = runPlugin(tree);
    ...
    it("blocks eval() calls", () => {
      const tree = programWith(
        exprStmt(callExpr(id("eval"), [literal("alert(1)")]))
      );
      const result = runPlugin(tree);
      expect(result.passed).toBe(false);
    ...
    it("blocks localStorage access", () => {
      const tree = programWith(
        exprStmt(
          callExpr(memberExpr(id("localStorage"), "getItem"), [literal("key")])
        )
      );
      const result = runPlugin(tree);
      expect(result.passed).toBe(false);
    ...
    it("blocks navigator.sendBeacon()", () => {
      const tree = programWith(
        exprStmt(
          callExpr(memberExpr(id("navigator"), "sendBeacon"), [
            literal("url"),
            literal("data"),
          ])
        )
    ...
    it("blocks sessionStorage access", () => {
      const tree = programWith(
        exprStmt(
          callExpr(memberExpr(id("sessionStorage"), "setItem"), [
            literal("k"),
            literal("v"),
          ])
        )
  ...
  describe("allowedExpressions config", () => {
    it("blocks console by default", () => {
      const tree = programWith(
        exprStmt(callExpr(memberExpr(id("console"), "log"), [literal("debug")]))
      );
    ...
    it("blocks fetch by default but allows with allowedExpressions", () => {
      const tree = programWith(
        exprStmt(callExpr(id("fetch"), [literal("url")]))
      );
    ...
    it("allows window when explicitly permitted", () => {
      const tree = programWith(
        exprStmt(memberExpr(id("window"), "innerWidth"))
      );
  ...
  describe("edge cases", () => {
    it("blocks deeply nested member expression (window.location.href)", () => {
      const tree = programWith(
        exprStmt(memberExpr(memberExpr(id("window"), "location"), "href"))
      );
      const result = runPlugin(tree);
      expect(result.passed).toBe(false);
      expect(result.error).toContain("window");
