  ...
  badge: z
    .union([
      ...
      z.object({
        text: z.string(),
        variant: z
          .enum(["info", "success", "warning", "danger", "default"])
          .optional(),
      }),
    ])
    .optional(),
...
export function parseCodeMeta(info: string): CodeMeta {
  ...
  if (/\b(?:showLineNumbers|lineNumbers)\b/.test(remaining)) {
    ...
    remaining = remaining
      .replace(/\b(?:showLineNumbers|lineNumbers)\b/, "")
      .trim();
  }
...
export function enhanceCodeBlock(html: string, meta: CodeMeta): string {
  ...
  if (meta.highlightLines && meta.highlightLines.length > 0) {
    let lineIndex = 0;
    result = result.replace(/<span class="line/g, (match) => {
      lineIndex++;
...
function getTwoslashTransformer(): Promise<any> {
  if (!twoslashTransformerPromise) {
    twoslashTransformerPromise = import("@shikijs/twoslash")
      ...
      .catch(() => {
        console.warn(
          "[tome] @shikijs/twoslash not available — twoslash annotations will be skipped."
        );
        return null;
      });
  }
...
function createCodeBlockTransformer(
...
export async function processMarkdown(
  ...
  if (mathOptions?.math) {
    try {
      ...
      console.warn(
        "[tome] remark-math not installed — math blocks will not be processed. Install with: pnpm add remark-math rehype-katex katex"
      );
    }
  }
  ...
  const transformCodeBlocks = createCodeBlockTransformer(
    highlighter,
    codeMetas,
    twoslashFactory
  );
