    ...
    >
      <div
        style={{
          ...
          border: "1px solid var(--bd, #ddd9d0)",
          borderRadius: 12,
          padding: 32,
          width: 420,
          maxWidth: "90vw",
...
function nav(to: string) {
  const p = to.startsWith(BASE) ? to : `${BASE}${to}`;
...
export default function EditorPage({
  ...
  const loadPages = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api<PageEntry[]>(
        `/api/editor/pages?projectSlug=${slug}`,
        { token }
      );
      const pageList = Array.isArray(data) ? data : [];
      setPages(pageList);
      if (pageList.length > 0 && !selectedId) {
        setSelectedId(pageList[0].id);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const page = await api<PageEntry>(`/api/editor/pages/${selectedId}`, {
          token,
        });
        if (cancelled) {
          return;
  ...
  const saveContent = useCallback(
    async (content: string, title: string, description: string) => {
      if (!selectedId) {
        return;
      }
      setSaveStatus("saving");
      try {
        await api(`/api/editor/pages/${selectedId}`, {
          method: "PUT",
          body: { content: sanitizeEditorContent(content), title, description },
          token,
        });
        setSaveStatus("saved");
        setPages((prev) =>
          prev.map((p) =>
            p.id === selectedId
              ? {
                  ...p,
                  status: p.status === "published" ? "changed" : p.status,
                  title,
                  description,
                }
              : p
  ...
  const debouncedSave = useCallback(
    (content: string, title: string, description: string) => {
      ...
      saveTimerRef.current = setTimeout(
        () => saveContent(content, title, description),
        2000
      );
    },
    [saveContent]
  ...
  const handleEditorChange = useCallback(
    (md: string) => {
      setEditorContent(md);
      debouncedSave(md, editTitle, editDescription);
    },
    [debouncedSave, editTitle, editDescription]
  );
  const handleTitleChange = (v: string) => {
    setEditTitle(v);
    debouncedSave(editorContent, v, editDescription);
  };
  const handleDescriptionChange = (v: string) => {
    setEditDescription(v);
    debouncedSave(editorContent, editTitle, v);
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      const result = await api<{
        ...
        method?: string;
        commitSha?: string;
      }>(`/api/editor/pages/${selectedId}/publish`, { method: "POST", token });
      setPages((prev) =>
        prev.map((p) =>
          p.id === selectedId ? { ...p, status: "published" } : p
        )
      );
      if (result.method === "github" && result.commitSha) {
        setSaveStatus(
          `Published & committed to GitHub (${result.commitSha.slice(0, 7)})`
        );
      } else {
  ...
  const handleNewPage = async (path: string, title: string) => {
    try {
      const page = await api<PageEntry>("/api/editor/pages", {
        method: "POST",
        body: { projectSlug: slug, path, title },
        token,
      });
  ...
  const handleDeletePage = async (pageId: string) => {
    ...
    try {
      ...
      if (selectedId === pageId) {
        const rem = pages.filter((p) => p.id !== pageId);
  ...
  return (
    <div className="rv" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {selectedPage && (
            <button
              className="btn-primary"
              disabled={publishing || selectedPage.status === "published"}
      ...
      {/* Two-panel layout */}
      <div
        style={{ display: "flex", gap: 20, minHeight: "calc(100vh - 220px)" }}
      >
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      background: active
                        ? "var(--coralD, rgba(139,58,47,0.08))"
                        : "transparent",
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        onMouseOver={(e) =>
                          (e.currentTarget.style.color = "var(--red, #b91c1c)")
                        }
                        style={{
                          background: "none",
                          border: "none",
        ...
        >
          {selectedPage ? (
            ...
            >
              <FileIcon />
              <span style={{ fontSize: 14 }}>Select a page to edit</span>
