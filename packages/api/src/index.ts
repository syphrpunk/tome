...
app.use("*", async (c, next) => {
  ...
  if (row?.sso_enabled === 1) {
    ...
    if (ssoToken) {
      const ssoResult = await validateSsoSession(
        c.env.SSO_SESSION_SECRET,
        ssoToken,
        slug
      );
      if (ssoResult && row.id) {
  ...
  if (row?.password_required === 1) {
    ...
    const tokenSlug = await validateSessionToken(
      sessionToken,
      c.env.SSO_SESSION_SECRET
    );
    if (tokenSlug !== slug) {
...
app.use(
  ...
  cors({
    origin: (origin) => {
      if (!origin) {
        return "*"; // Allow non-browser requests (CLI, cURL)
      }
      const allowedOrigins = ["https://tome.center", "https://www.tome.center"];
