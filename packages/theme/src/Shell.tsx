...
import { type PresetName, THEME_PRESETS } from "./presets.js";
...
const PencilIcon = () => (
  <Icon d="M17 3a2.83 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" size={13} />
);
...
function AlgoliaSearchModal({
  ...
  const [DocSearchComponent, setDocSearchComponent] =
  ...
  const extractPageId = useCallback(
    (url: string): string => {
      try {
  ...
  if (loadFailed) {
    return (
      ...
      >
        ...
        >
          Algolia DocSearch is not available. Install @docsearch/react to enable
          it.
        </div>
      </div>
    );
...
const GlobeIcon = () => (
  <Icon
    d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18ZM3.6 9h16.8M3.6 15h16.8M12 3a15 15 0 0 1 4 9 15 15 0 0 1-4 9 15 15 0 0 1-4-9 15 15 0 0 1 4-9Z"
    size={14}
  />
);
...
function getBreadcrumbs(
  ...
  for (const section of navigation) {
    ...
    const trail: BreadcrumbItem[] = [
      { label: section.section, href: firstItem?.urlPath ?? null },
    ];
...
export function Shell({
  ...
  const dir: "ltr" | "rtl" =
    dirProp || i18n?.localeDirs?.[resolvedLocale] || "ltr";
  const isRtl = dir === "rtl";
  ...
  const [bannerDismissed, setBannerDismissed] = useState(() => {
    ...
    try {
      const hash = Array.from(config.banner.text)
  ...
  const isOldVersion =
    versioning && currentVersion && currentVersion !== versioning.current;
  ...
  const baseTokens =
    THEME_PRESETS[preset]?.[isDark ? "dark" : "light"] ||
  ...
  useEffect(() => {
    if (!showToc || filteredHeadings.length < 2) {
      return;
    }
  ...
  useLayoutEffect(() => {
    if (!(htmlContentRef.current && pageHtml)) {
      return;
    }
  ...
  const prev =
    idx > 0
      ? allNavPages[idx - 1]
      : (allNavPages[allNavPages.length - 1] ?? null);
  const next =
    idx < allNavPages.length - 1
      ? allNavPages[idx + 1]
      : (allNavPages[0] ?? null);
          ...
          if (isNavGroup(entry)) {
            return (
              ...
              >
                ...
                >
                  {expanded.includes(entry.section) ? (
                    <ChevDown />
                  ) : (
                    <ChevRight />
                  )}
    ...
    return (
      <button
        ...
        style={{
          ...
          border: "none",
          borderRadius: 0,
          [isRtl ? "borderRight" : "borderLeft"]: active
            ? "2px solid var(--ac)"
      ...
      >
        ...
        {p.badge &&
          (() => {
            const bc =
              badgeColors[p.badge!.variant || "default"] || badgeColors.default;
            return (
              <span
                style={{
                  fontSize: 10,
  ...
  const bannerIsInternal = bannerLink
    ? bannerLink.startsWith("#") ||
      (basePath && bannerLink.startsWith(basePath + "/"))
    : false;

  return (
    <div
      className="tome-grain"
      dir={dir}
      style={{
    ...
    >
      ...
      {config.banner?.text && !bannerDismissed && (
        ...
        >
          {config.banner.link ? (
            <a
              href={
                bannerIsInternal && bannerLink!.startsWith("#")
                  ? basePath + "/" + bannerLink!.slice(1)
                  : bannerLink!
              }
              {...(bannerIsInternal
                ? {}
              ...
              onClick={
                bannerIsInternal
                  ? (e: React.MouseEvent) => {
                      ...
                      const pageId = bannerLink!.startsWith("#")
                        ? bannerLink!.slice(1)
                        : bannerLink!.replace(new RegExp("^" + bp + "/?"), "");
      ...
      {searchOpen &&
      config.search?.provider === "algolia" &&
      config.search.appId &&
      ...
      config.search.indexName ? (
        ...
        <SearchModal
          aiSearch={
            config.search?.ai && config.ai?.enabled
              ? {
      ...
      >
        ...
        {overrides?.Sidebar ? (
          ...
          >
            ...
            <nav
              style={{ flex: 1, overflow: "auto", padding: "4px 10px 20px" }}
            >
              {navigation.map((sec) => (
                ...
                {versioning.versions.map((v) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const targetId =
                        v === versioning.current ? "index" : `${v}/index`;
                      onNavigate(targetId);
                    }}
                    style={{
                      flex: 1,
                      ...
                      textAlign: "center",
                      background:
                        v === (currentVersion || versioning.current)
                          ? "var(--acD)"
                          : "var(--sf)",
                      border: "1px solid var(--bd)",
                      borderRadius: 2,
            ...
            >
              ...
              >
                {typeof __TOME_VERSION__ !== "undefined" && __TOME_VERSION__
                  ? `v${__TOME_VERSION__}`
                  : "v0.1.0"}
              </span>
            </div>
          ...
          {overrides?.Header ? (
            ...
            >
              ...
              {mobile ? (
                ...
                >
                  {flattenNav(navigation).find((p) => p.id === currentPageId)
                    ?.title || ""}
                </span>
              ) : (
                <div
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  {config.topNav.map((link) => {
                    const isInternal =
                      ...
                      (basePath && link.href.startsWith(basePath + "/"));
                    const isExternal = !isInternal;
                    return (
                      <a
                        href={
                          isInternal && link.href.startsWith("#")
                            ? basePath + "/" + link.href.slice(1)
                            : link.href
                        }
                        key={link.label}
                        {...(isExternal
                        ...
                        onClick={
                          isInternal
                            ? (e: React.MouseEvent) => {
                                e.preventDefault();
                                const pageId = link.href.startsWith("#")
                                  ? link.href.slice(1)
                                  : link.href.replace(
                                      new RegExp(
                                        "^" +
                  ...
                  <span
                    style={{ width: 1, height: 16, background: "var(--bd)" }}
                  />
                </div>
              )}
              ...
              {i18n && i18n.locales.length > 1 && !mobile && (
                <div style={{ position: "relative" }}>
                  ...
                  >
                    <GlobeIcon />
                    {i18n.localeNames?.[currentLocale || i18n.defaultLocale] ||
                      currentLocale ||
                      i18n.defaultLocale}
                    <ChevDown />
                  </button>
                  {localeDropdownOpen && (
                    ...
                    >
                      {i18n.locales.map((locale) => {
                        const isActive =
                          locale === (currentLocale || i18n.defaultLocale);
                        const displayName =
                          i18n.localeNames?.[locale] || locale;
                        ...
                        if (
                          ...
                          currentPageId.startsWith(`${activeLocale}/`)
                        ) {
                          basePageId = currentPageId.slice(
                            activeLocale.length + 1
                          );
                        }
                        const targetId =
          ...
          >
            <main
              style={{
                ...
                maxWidth: mobile
                  ? "100%"
                  : apiManifest || asyncApiManifest
                    ? 1100
                    : 760,
                padding: mobile ? "24px 16px 60px" : "40px 48px 80px",
                margin: "0 auto",
                minWidth: 0,
              }}
            >
                ...
                >
                  {breadcrumbs.map((crumb, i) => (
                    <React.Fragment key={i}>
                      {i > 0 && (
                        <span style={{ color: "var(--tx2)", opacity: 0.5 }}>
                          {"\u203A"}
                        </span>
                      )}
                      {i < breadcrumbs.length - 1 && crumb.href !== null ? (
                        <a
                          ...
                          onClick={(e: React.MouseEvent) => {
                            ...
                            const page = flattenNav(navigation).find(
                              (p) => p.urlPath === crumb.href
                            );
                            if (page) {
                              onNavigate(page.id);
                            }
              ...
              <div style={{ borderTop: "1px solid var(--bd)", paddingTop: 28 }}>
                ...
                {(apiManifest || asyncApiManifest) &&
                (ApiReferenceComponent || AsyncApiReferenceComponent) ? (
                  <>
                    {apiManifest && ApiReferenceComponent && (
                      <ApiReferenceComponent
                        baseUrl={apiBaseUrl}
                        manifest={apiManifest}
                        playgroundAuth={apiAuth}
                          ...
                          >
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    (window as any).__tome?.trackFeedback(
                                      currentPageId,
                                      feedbackRating[currentPageId],
                                      feedbackComment
                                    );
                                    try {
                                      localStorage.setItem(
                                        `tome-feedback-${currentPageId}`,
                                        feedbackRating[currentPageId]
                                      );
                                    } catch {}
              ...
              : showToc &&
                ...
                wide && (
                  <aside
                    ...
                    style={{
                      width: 200,
                      padding: isRtl ? "40px 0 40px 16px" : "40px 16px 40px 0",
                      position: "sticky",
                      top: 0,
      ...
      {config.ai?.enabled && (
        <AiChat
          apiKey={
            typeof __TOME_AI_API_KEY__ !== "undefined" && __TOME_AI_API_KEY__
              ? __TOME_AI_API_KEY__
              : undefined
          }
          context={
    ...
    aiDebounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const searchContext = results
          ...
          .map((r) => `Page: ${r.title}\n${r.excerpt || ""}`)
          .join("\n\n");
        const fullContext = aiSearch.context
          ? `${searchContext}\n\n---\n\n${aiSearch.context}`
          : searchContext;
      ...
      >
        ...
        {aiSearch && q.length >= 3 && (aiLoading || aiAnswer) && (
          ...
          >
            ...
            {aiLoading ? (
              <div style={{ color: "var(--txM)", fontStyle: "italic" }}>
                Thinking...
              </div>
            ) : aiAnswer ? (
              <div>{aiAnswer}</div>
